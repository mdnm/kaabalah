...
- `TREE_SPHERE_IDS`
- `TREE_PATH_IDS`
- Types: `TreeSvgOptions`, `TreeSvgPalette`, `TreeSvgCustomPalette`, `TreeSvgDaathLayer`, `TreeSvgViewBox`, `TreeLayout`, `TreeLayoutMap`, `TreeLayoutPath`, `TreeLayoutCoordinate`, `TreeSphereName`, `TreeSphereId`, `TreePathId`
...
```typescript
import { generateTreeSvg, TREE_SPHERE_IDS, TREE_PATH_IDS } from 'kaabalah/visual';
...
const svg = generateTreeSvg({
  ...
  palette: {
    ...
    // Override a specific path color
    pathColors: {
      [id(KaabalahTypes.PATH, "1")]: "#ffd700",
    },
    // Override a specific sphere fill
    sphereFills: {
      [id(KaabalahTypes.SPHERE, "Tiphareth")]: "#f5c518",
    },
  },
});
```
