...
Planet glyphs are rendered in a stable Planet Band. Exact ticks, true anchors, aspect endpoints, and house membership stay tied to true longitude. Degree, sign, minutes, and retrograde details are rendered from independent Position Rails by default. Extra point layers can override those rails independently, so transit overlays can use a lighter label preset without changing the natal wheel. Override `palette.glyphHalo` when the automatic halo color should not match the wheel background.
...
const svg = generateAstroWheelSvg(chart, {
  layout: {
    ...
    rails: "compact",             // "full" | "compact" | "glyph-only" | partial visibility map
  },
});
```

Planet glyphs may be angularly spaced for dense clusters, but zodiacal order is preserved and displacement is capped. The model exposes both `trueAnchor` and `glyphPosition`; generated SVG draws a short band leader when those positions differ. Aspect lines connect true anchors on the structural aspect boundary, not displaced glyphs. That boundary remains visible when `aspects: false`, so house cusp spokes still end on the same inner wheel circle.

Boundary Notches are local relief marks in the Planet Band. They are generated only when a glyph is cusp-adjacent or displaced across a cusp conflict; position text does not create notches and house cusp lines do not move.

#### Position rails

Static SVG defaults to the full rail set. Compact app surfaces can hide detail rails while preserving the same anchors, houses, and glyph positions:
...
`pointLayers` and `aspectLayers` let you compose transits, synastry, or any custom calculated points over a base chart without rebuilding the SVG. Extra point layers share the base chart's Ascendant orientation and can be placed inside, on, or outside the normal planet ring. External point layers render only the degree rail by default so transit overlays stay readable without crowding neighbouring points with minute or sign labels. Set `pointLayers[].rails` to override that layer-specific default.
...
Set `chartB` to compute **cross-chart** aspects between two charts (transit-to-natal, synastry): aspects are formed between `chart`'s planets (side A) and `chartB`'s planets (side B), and each endpoint anchors to the layer named by `pointLayerIdA` / `pointLayerIdB`. Use `pointLayerIdA` / `pointLayerIdB` with explicit `edges` when you have pre-computed cross-chart lines. The shorthand `pointLayerId: "transit"` draws aspects within a single overlay layer (transit-to-transit).
...
model.points;         // true anchors, displayed glyph positions, rails, hit targets
model.pointLayers;    // grouped birth/transit/synastry/custom point layers
model.houseCusps;     // cusp line geometry in SVG units
model.positionRails;  // rail radii and visibility state
model.boundaryNotches;// local planet-band notches
model.aspectLines;    // true-anchor aspect line geometry and colors
...
Each `AstroWheelPoint` contains:

- `longitude`, `trueAnchor`, and `trueAngle` for exact chart position.
- `displayLongitude`, `glyphPosition`, and `displayAngle` for the displayed Planet Band glyph.
- `house`, `nearestCuspHouse`, and `nearestCuspDistanceDegrees` from true longitude against exact house cusps.
- `railLabels` for degree/sign/minutes/retrograde rendering.
- `hitTarget` for pointer interaction and `leaderArc` when the glyph is displaced.
...
interface ArcheometerSvgOptions {
  width?: number | string;
  height?: number | string;
  viewBox?: ArcheometerSvgViewBox;         // default: 0 0 912 912
  background?: string | "transparent";    // default: "transparent"
...
Use `getArcheometerRenderModel()` when you need the normalized ring geometry, resolved palette, default correspondence tables, and per-element geometry without serializing SVG.
...
model.rings;                         // normalized ring radii
model.geometry.utterancePoints;       // anchors and hit targets for letters/numbers
model.geometry.triangles;             // trigone vertices, centroids, paths, hit targets
model.geometry.triangleLabels;        // label anchors
model.geometry.musicalNotes;          // note anchors
model.geometry.zodiacSigns;           // zodiac glyph anchors and hit targets
model.geometry.planetaryPoints;       // planet glyph anchors and hit targets
model.geometry.degreeTicks;           // degree crown anchors
model.geometry.rays;                  // white ray line geometry
model.geometry.center;                // solar center anchor and hit target
```

Generated SVG consumes the same model-level geometry contract: source data remains available on each element, while `anchor` and `hitTarget` let React, Canvas, or app-specific renderers place tooltips and interaction zones without parsing SVG markup.
...
import { generateTreeSvg, getRouteActivations } from 'kaabalah/visual';

const route = getTreeTopology().getRoute("lightning");
const activations = route ? getRouteActivations(route, { state: "selected" }) : [];
const svg = generateTreeSvg({ background: "transparent", activations });
```
