...
### `shuffleTarotDeck(cards, includeInvertedCards?, shuffleCount?, shuffleDelay?, rng?)`
...
async function shuffleTarotDeck(
  ...
  rng?: () => number                // default: Math.random — inject for tests/replay only
): Promise<TarotCard[]>
```
...
The default `shuffleDelay` of 300 ms per pass (~1.8 s total) is a UX affordance for human-facing draws. Pass `0` for machine-driven flows — the CLI does this automatically in `--json` mode. The `rng` parameter exists so tests can prove permutation behavior deterministically; production draws should keep the `Math.random` default.
...
// No artificial delay (e.g. server-side or pipeline use):
const instant = await shuffleTarotDeck(ARKANNUS, false, 6, 0);
```
