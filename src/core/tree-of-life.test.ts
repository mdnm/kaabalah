...
import { BaseNode, id, KaabalahTypes, LetterTypes, NumerologyTypes, TarotTypes, WesternAstrologyTypes } from './types';

describe('TreeOfLife', () => {
  it('should add nodes and links, retrieving relations correctly', () => {
    const treeOfLife = new TreeOfLife();

    const kether = treeOfLife.addSphere({ sphere: SPHERES.KETHER, data: SPHERES_DATA.KETHER, relatedNumber: 1 });
    const chokhmah = treeOfLife.addSphere({ sphere: SPHERES.CHOKHMAH, data: SPHERES_DATA.CHOKHMAH, relatedNumber: 2 });

    const path11 = treeOfLife.addPath({ leftSphere: kether, rightSphere: chokhmah, relatedNumber: 1 });
  ...
  it('should throw an error when trying to link unknown nodes', () => {
    ...
    expect(() => treeOfLife.link(id(KaabalahTypes.SPHERE, "Kether"), id(KaabalahTypes.SPHERE, "Unknown"))).toThrow();
  });
  ...
  it('should correctly link nodes', () => {
    ...
    const chokhmah = treeOfLife.addSphere({ sphere: SPHERES.CHOKHMAH, data: SPHERES_DATA.CHOKHMAH, relatedNumber: 2 });
    const binah = treeOfLife.addSphere({ sphere: SPHERES.BINAH, data: SPHERES_DATA.BINAH, relatedNumber: 3 });

    const path14 = treeOfLife.addPath({ leftSphere: chokhmah, rightSphere: binah, relatedNumber: 4 });

    treeOfLife.addLetters({ path: path14, letters: [{ letter: "Aleph", type: LetterTypes.HEBREW_LETTER, data: HEBREW_LETTERS_DATA.ALEPH }, { letter: "A", type: LetterTypes.LATIN_LETTER }] });

    const air = treeOfLife.upsertNode(new BaseNode({ id: "Air", type: WesternAstrologyTypes.WESTERN_ELEMENT }));
    treeOfLife.link(path14, air);

    const jupiter = treeOfLife.upsertNode(new BaseNode({ id: "Jupiter", type: WesternAstrologyTypes.PLANET }));
    treeOfLife.link(path14, jupiter);

    const theEmperor = treeOfLife.addTarotArkAnnu({
      node: path14,
      tarotArkAnnu: "The Emperor",
      data: { type: "major", descriptiveData: [], },
      relatedNumber: 14,
    });
    ...
    expect(treeOfLife.related(path14)).toMatchObject([
      ...
      { id: id(LetterTypes.HEBREW_LETTER, "Aleph"), type: LetterTypes.HEBREW_LETTER },
    ...
    expect(treeOfLife.walk(theEmperor, 2, WesternAstrologyTypes.WESTERN_ELEMENT)).toMatchObject([
      { id: air, type: WesternAstrologyTypes.WESTERN_ELEMENT },
    ]);
  });
